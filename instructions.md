...
1. **Per-flow skill** at `.claude/skills/parallel-implement-flow/SKILL.md` — gives the agent a strong description match on "implement all task flows for X in parallel" / "build everything for X in parallel" requests, with Claude-specific Phase 2 guidance (skip `git worktree add`, dispatch via Agent tool with `subagent_type: parallel-implementer`).
2. **Custom subagent** at `.claude/agents/parallel-implementer.md` with `isolation: worktree` in its frontmatter — when parallel-implement-flow calls this subagent type, Claude Code auto-creates a fresh worktree under `.claude/worktrees/parallel-implementer-<id>/` and runs the subagent there. This replaces the manual `git worktree add` loop the canonical flow describes.
3. **Slash command** at `.claude/commands/parallel-implement-flow.md` for `/parallel-implement-flow <task-name>` one-keystroke invocation.
...
| "implement all task flows for X in parallel" / "build everything for X in parallel" | `.ai-workflow/flows/parallel-implement-flow.md` |
