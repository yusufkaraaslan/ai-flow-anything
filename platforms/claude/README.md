...
# 2. Per-flow skill: parallel-implement-flow
mkdir -p .claude/skills/parallel-implement-flow
ln -s ../../../.ai-workflow/platforms/claude/flow-skills/parallel-implement-flow/SKILL.md .claude/skills/parallel-implement-flow/SKILL.md

# 3. Custom subagent for parallel-implement-flow's parallel implementers
mkdir -p .claude/agents
ln -s ../../.ai-workflow/platforms/claude/agents/parallel-implementer.md .claude/agents/parallel-implementer.md

# 4. Slash command for parallel-implement-flow
mkdir -p .claude/commands
ln -s ../../.ai-workflow/platforms/claude/commands/parallel-implement-flow.md .claude/commands/parallel-implement-flow.md
```
...
| `.claude/skills/parallel-implement-flow/SKILL.md` | Per-flow skill — strong description match for "implement {task} in parallel" / "build everything for {task}" |
...
- **Commands** (`.claude/commands/{name}.md`) — slash commands the developer types directly (e.g. `/parallel-implement-flow user-auth`).
...
**Parallel-implement-flow specifically** uses all four wrapper types: the master skill (for ambiguous requests), the per-flow skill (for strong "implement X in parallel" matches), the `parallel-implementer` subagent (for parallel execution with auto-worktrees), and the `/parallel-implement-flow` slash command (for direct invocation). See [`flow-skills/parallel-implement-flow/SKILL.md`](flow-skills/parallel-implement-flow/SKILL.md) for the parallel-subagent dispatch pattern.
...
rm -rf .ai-workflow \
  ...
  .claude/skills/parallel-implement-flow \
  .claude/agents/parallel-implementer.md \
  .claude/commands/parallel-implement-flow.md
```

This removes ai-flow-anything but leaves your project's own `flow-storage/` (your knowledge base) untouched. If you ran parallel-implement-flow previously, any leftover sub-agent branches (`parallel-implementer-*`) can be pruned with `git branch -D` — they're independent of the wrapper install.
